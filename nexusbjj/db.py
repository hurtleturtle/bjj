...
class Database:
    ...
    def add_user(self, email, password, admin_level='no'):
        query = 'INSERT INTO users (email, password, admin) VALUES (%s, %s, %s)'
        params = (email, generate_password_hash(password), admin_level)
        self.execute(query, params)
        self.commit()
    ...
    def get_attendance(self, from_date, to_date, user_id=None, class_id=None):
        ...
        query += 'classes.class_type, users.email FROM attendance '
